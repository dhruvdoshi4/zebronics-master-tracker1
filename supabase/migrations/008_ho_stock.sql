-- HO Stock consolidated report (run after 007_gms_tracker.sql).
...
create index if not exists ho_stock_snapshot_asin_idx
  on public.ho_stock_snapshot (upload_id, asin)
  where asin <> '';

create index if not exists ho_stock_snapshot_fsn_idx
  on public.ho_stock_snapshot (upload_id, fsn)
  where fsn <> '';
