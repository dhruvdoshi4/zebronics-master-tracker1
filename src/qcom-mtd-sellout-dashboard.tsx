...
function MomMonthTooltip({
  ...
  row: QcomMomSeriesRow;
  showChannelBreakdown: boolean;
  channelsActive: Record<QcomMarketplace, boolean>;
}) {
  const channelLine =
    showChannelBreakdown && row.channelUnits
      ? formatQcomChannelUnitsLine(row.channelUnits, channelsActive)
      : null;
  ...
  return (
    <div className="min-w-[240px] rounded-xl border-2 border-zinc-200 bg-white px-4 py-3 shadow-lg">
      ...
      <p className="mt-2 text-sm font-semibold text-zinc-700">
        ...
        {channelLine ? (
          <span className="block text-xs font-semibold text-zinc-500">({channelLine})</span>
        ) : null}
      </p>
      ...
      <div className="space-y-5">
        ...
        <Card className="border-zinc-200 p-5 shadow-sm">
            <div className="mb-3">
              ...
              <p className="mt-0.5 text-sm font-medium text-zinc-500">
                Each bar is this FY. Completed months are full month; ongoing month is MTD. Hover
                for prior year and growth %.
              </p>
            </div>

            <div className="h-[300px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={momChartSeries} margin={{ top: 12, right: 8, left: 4, bottom: 4 }}>
                  <CartesianGrid stroke={CHART_GRID_STROKE} strokeDasharray="3 3" vertical={false} />
                  <XAxis
                  ...
                  <Tooltip
                    content={({ active, payload }) => {
                      if (!active || !payload?.length) return null;
                      const row = payload[0]?.payload as QcomMomSeriesRow | undefined;
                      if (!row) return null;
                      return (
                        <MomMonthTooltip
                          row={row}
